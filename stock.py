...
def _parse_resp(status, content):
    return jsonify(content) if status == 0 and content else Response(content, status=400)
...
def get_trading_days():
    ...
    return _parse_resp(status, content)


@app.route("/stock/get_stock_basicinfo")
def get_stock_basicinfo():
    market = request.args.get('market').upper()
    status, content = api.get_stock_basicinfo(market)
    return _parse_resp(status, content)
...
if __name__ == "__main__":
    ...
    def _get_index(e1):
        res = list(filter(lambda e2: (e2[1] - e1[1]) / e1[1] >= 0.3, [(i, e0) for i, e0 in enumerate(list(kline[:, 2]))][e1[0]+1:]))
